...
| <a name="input_content_frame_ancestors"></a> [content\_frame\_ancestors](#input\_content\_frame\_ancestors) | Level of CSP header's frame-ancestors control. 'none' means there are no valid parents that may embed the page (Content-Security-Policy: frame-ancestors 'none'), 'same\_origin' allows same-origin embedding only (Content-Security-Policy: frame-ancestors 'self'), 'all' allows all iframes (no Content-Security-Policy header) | `string` | `"none"` | no |
